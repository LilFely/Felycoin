...
To build Bitcoin Core without Qt, unload or disable the `felycoin-qt`, `libfelycoin_qt` and `test_felycoin-qt` projects.
...
```cmd
msbuild build_msvc\felycoin.sln -property:Configuration=Release -maxCpuCount -verbosity:minimal
```

Alternatively, open the `build_msvc/felycoin.sln` file in Visual Studio.
...
To check if `felycoind` has `RandomizedBaseAddress` enabled or disabled run

```
.\dumpbin.exe /headers src/felycoind.exe
```

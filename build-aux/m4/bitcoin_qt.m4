...
dnl Output: If qt version is auto, set felycoin_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "$felycoin_qt_want_version" = "auto" && test "$felycoin_qt_force" != "yes"; then
    if test "$felycoin_enable_qt" != "no"; then
      AC_MSG_WARN([$1; felycoin-qt frontend will not be built])
    fi
    felycoin_enable_qt=no
    felycoin_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "$felycoin_enable_qt" != "no" && test "$felycoin_qt_want_version" != "no"; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build felycoin-qt GUI (default=auto)])],
    [
     felycoin_qt_want_version=$withval
     if test "$felycoin_qt_want_version" = "yes"; then
       felycoin_qt_force=yes
       felycoin_qt_want_version=auto
     fi
    ],
    [felycoin_qt_want_version=auto])
  ...
  AS_IF([test "$with_gui" = "qt5_debug"],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); felycoin_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl Outputs: felycoin_enable_qt, felycoin_enable_qt_dbus, felycoin_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "$felycoin_cv_static_qt" = "yes"; then
  ...
  BITCOIN_QT_CHECK([
    felycoin_enable_qt=yes
    felycoin_enable_qt_test=yes
    if test "$have_qt_test" = "no"; then
      felycoin_enable_qt_test=no
    fi
    felycoin_enable_qt_dbus=no
    if test "$use_dbus" != "no" && test "$have_qt_dbus" = "yes"; then
      felycoin_enable_qt_dbus=yes
    fi
  ...
  if test $felycoin_enable_qt = "yes"; then
    AC_MSG_RESULT([$felycoin_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$felycoin_enable_qt])
  fi
...
dnl Output: felycoin_cv_static_qt=yes|no
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, felycoin_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [felycoin_cv_static_qt=yes],
      [felycoin_cv_static_qt=no])
    ])
])
